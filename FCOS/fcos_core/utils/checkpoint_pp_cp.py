...
class DetectronCheckpointer(Checkpointer):
    ...
    def save(self, name, iterations, map_best_result, **kwargs):
        if not self.save_dir:
            return
        ...
        if self.optimizer is not None:
            ##------------------pp-----------------
            data["optimizer_backbone"] = self.optimizer["backbone"].state_dict()
            data["optimizer_fcos"] = self.optimizer["fcos"].state_dict()
        ...
        if self.scheduler is not None:
            ...
            data["scheduler_backbone"] = self.scheduler["backbone"].state_dict()
            data["scheduler_fcos"] = self.scheduler["fcos"].state_dict()
    ...
    def load(self, f=None, load_dis=True, load_opt_sch=False):
        ##------------pp:取消从last_checkpoint文件（指定路径）加载----------
        # if self.has_checkpoint():
        #     # override argument with existing checkpoint
        #     f = self.get_checkpoint_file()
        if not f:
        ...
        # print('checkpoint["iterations"]: ', checkpoint["iterations"])
        # print('checkpoint.keys(): ', "iterations" in list(checkpoint.keys()))
        iter = 0
        map_best = 0
        if "iterations" in list(checkpoint.keys()):
            iter = checkpoint["iterations"]

        if "map_best" in list(checkpoint.keys()):
            map_best = checkpoint["map_best"]

        print('map_best: ', map_best)
        # return any further checkpoint data
        return checkpoint, iter, map_best
