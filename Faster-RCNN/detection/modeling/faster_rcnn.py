...
class FasterRCNN(nn.Module):
    def __init__(self, cfg):
        ...
        # num_bins = 10
        num_bins = cfg.MODEL.BIN_ADV.NUM_BIN
    ...
    def forward(self, images, img_metas, targets=None, t_images=None, t_img_metas=None):
        ...
        if self.enable_adaptation and self.training and t_images is not None:
            ...
            _, _, t_proposals, t_box_features, t_roi_features, t_class_logits, t_bin_box_regression = self.box_head(t_features, t_proposals,t_img_metas, targets=None)
            ...
            ##特征反转
            ### box_features_bin = self.grad_reverse(box_features_bin)
            ### t_box_features_bin = self.grad_reverse(t_box_features_bin)

            box_features_bin = self.grad_reverse(box_features)
            t_box_features_bin = self.grad_reverse(t_box_features)

            ebox_features_reg_bin_adv = self.extractor_regress_bin_adv(box_features_bin)
            ...
            t_ebox_features_reg_bin_adv = self.extractor_regress_bin_adv(t_box_features_bin)
            ...
            adv_loss.update({
                'loss_adv_box_bin': 0.5 * loss_adv_box_bin  ##sim10k 44.3
            })
