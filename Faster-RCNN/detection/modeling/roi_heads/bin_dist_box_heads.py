...
class VGG16BoxPredictor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.bin_bbox_pred = nn.Linear(in_channels, num_bins * 4)
    ...
    def forward(self, box_features):
        ...
        bin_mbox_features_reg = self.regressor_bin(ebox_features_reg)
...
def fastrcnn_loss(class_logits, box_regression, bin_box_regression, labels, regression_targets, bin_regression_targets):
    ...
    bin_regression_targets = bin_regression_targets[sampled_pos_inds_subset]

    ###保存各bin分布结果
    bin_dx = bin_regression_targets[:,0]
    all_num_per_bin = np.zeros((4,10))

    for i in range(4):
        bin_dx = bin_regression_targets[:, i]
        for a_bin in range(1,11):
            idx = bin_dx == a_bin
            idx_sum = sum(idx)
            all_num_per_bin[i, a_bin - 1] = idx_sum


    txt_path = '/data/piaozhengquan/projects/DAOD/MGADA-master/Faster-RCNN/bin_distri/bin.txt'
